...
function cross_validation(seed,CV)
    ...
    for cv=1:CV
        ...
        X_feature_num=2;
        train_X_cell{1,1}=PCPseDNCFeature_LNCRNA;
        test_X_cell{1,1}=PCPseDNCFeature_LNCRNA;
        train_X_cell{1,1}(test_index,:)=0;
        test_X_cell{1,1}(train_index,:)=0;

        train_X_cell{2,1}=SCPseDNCFeature_LNCRNA;
        test_X_cell{2,1}=SCPseDNCFeature_LNCRNA;
        train_X_cell{2,1}(test_index,:)=0;
        test_X_cell{2,1}(train_index,:)=0;

        for j=1:X_feature_num
            train_lncRNA_simi_cell{j,1}=fast_LNS_caculate(train_X_cell{j,1},size(train_X_cell{j,1},1));
        end
        score_matrix=LPI_pred(train_X_cell,train_interaction_matrix,train_lncRNA_simi_cell,test_X_cell);    %prediction results for new lncRNAs
